...
_curTarget = [_this,0,ObjNull,[ObjNull]]
call BIS_fnc_param;
if(isNull _curTarget) exitWith {closeDialog 0;};
_isVehicle = if((_curTarget isKindOf "landVehicle") OR (_curTarget isKindOf "Ship") OR (_curTarget isKindOf "Air")) then {true}
else{false};
...
else
{
	if(_curTarget isKindOf "Ship") then
	{
		...
		if(_curTarget isKindOf "Ship" && {local _curTarget} && {count crew _curTarget == 0}) then { _Btn2 ctrlEnable true; }
	...
	else
	{
		...
		if(count crew _curTarget == 0 && {canMove _curTarget}) then { _Btn2 ctrlEnable false; }
		else { _Btn2 ctrlEnable true; };
	};
	if(typeOf _curTarget in
	[
		"B_Truck_01_mover_F",
	...
	then
	{
		...
		else { _Btn3 ctrlEnable true; };
	}
	else { _Btn3 ctrlShow false; };
