...
switch (true) do
{
	case (player distance (getMarkerPos "lead_1") < 50): {_mine = ["copper_unrefined",3];};
	case (player distance (getMarkerPos "uranium_1") < 50): {_mine = ["uraniumU",3];};
	case (player distance (getMarkerPos "iron_1") < 50): {_mine = ["iron_unrefined",3];};
	case (player distance (getMarkerPos "gold_1") < 50): {_mine = ["goldbarU",1];};
	case (player distance (getMarkerPos "diamond_1") < 50): {_mine = ["diamond_uncut",3];};
	case (player distance (getMarkerPos "oil_1") < 50) : {_mine = ["oil_unprocessed",3];};
