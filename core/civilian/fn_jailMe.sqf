...
[_bad,_time] spawn
{
	...
	if(_this select 0) then {sleep ( (_this select 1) * 0.5 );}
	else {sleep ( (_this select 1) * 0.2 );};
...
while {true} do
{
	if((round(_time - time)) > 0) then
	{
		...
		else {hintSilent format["Time Remaining:\n %1\n",_countDown];};
	};
	if(player distance (getMarkerPos "jail_marker") > 35) exitWith {_esc = true;};
