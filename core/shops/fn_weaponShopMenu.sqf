...
if(_exit) exitWith {};
_flag = switch(playerSide) do {case west: {"cop"}; default {"civ"};};
if(!(EQUAL(_flag,_shopSide))) exitWith {};
if(!(EQUAL(_level,-1))) then
{
	_flag = switch(playerSide) do {case west: {(_level <= (FETCH_CONST(life_copLevel)))}; default {true};};
...
if(!isClass(missionConfigFile >> "WeaponShops" >> (SEL(_this,3)))) exitWith {};
