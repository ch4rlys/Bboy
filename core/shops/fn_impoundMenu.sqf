...
for "_i" from 0 to (count life_vehicles)-1 do
{
	...
	if(_veh distance impound_obj < 50) then
	{
		...
		switch (true) do
		{
			case (_veh isKindOf "Car") : {ADD(_price, 350);};
			case (_veh isKindOf "Air") : {ADD(_price, 850);};
			case (_veh isKindOf "Ship") : {ADD(_price, 250);};
			case (_veh isKindOf "Tank") : {ADD(_price, 1250);};
		};
		if(_price > CASH) then
		{
			ADD(_price, 200);
